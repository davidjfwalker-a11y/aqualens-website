...
# I will extract just the section blocks by their comments to be safe!
def get_block(name):
    # Regex to find block between {/* ========== NAME ========== */} and the next {/* ==========
    match = re.search(r'\{/\*\s*==========\s*' + name + r'\s*==========\s*\*/\}(.*?)(?=\{/\*\s*==========|$)', orig, re.DOTALL)
    return match.group(0) if match else ""

nav = get_block("NAVIGATION")
ai = get_block("AI IDENTIFICATION")
health = get_block("HEALTH DIAGNOSTICS")
tank = get_block("TANK MANAGEMENT")
labs = get_block("LABS & PRO")
hardware = get_block("HARDWARE / FILTRATION")
features = get_block("FEATURES OVERVIEW")
gallery = get_block("SCREENSHOT GALLERY")
cta = get_block("CTA / DOWNLOAD")
...
new_content = prefix + nav + hero + ai + health + tank + labs + hardware + features + gallery + cta + footer + suffix
